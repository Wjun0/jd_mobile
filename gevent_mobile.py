...
class Jd_mobile(object):
    ...
    def parse(self,url):
        ...
        g_list = [gevent.spawn(self.parse_one,url,i) for i in range(1,count)]
        gevent.joinall(g_list)

        # for i in range(1,2):
        #     self.parse_one(url,i)
    ...
    def run(self):
        ...
        #使用协程方式运行
        # g_list= [gevent.spawn(self.parse,url) for url in detail_url_list[:2]]
        # gevent.joinall(g_list)
